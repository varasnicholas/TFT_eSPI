...
void TFT_eSPI::loadFont(String fontName, bool flash)
{
  /*
    ...
    After the bitmaps is:
       1 byte for font name string length (excludes null), this is the font name seen by Java when it's created
       a zero terminated character string giving the font name
       1 byte for Postscript name string length
       a zero/one terminated character string giving the Postscript font name
